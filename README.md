...
4. All __ride_id__ values have length 16, so there is no need to clean it.
...
6. The __started_at__ and __ended_at__ shows start and end time of the trip in YYYY-MM-DD hh:mm:ss UTC format. A new column ride_length can be created to find the total trip duration. There are 5360 trips with duration longer than a day and 122283 trips with less than a minute duration or have end time earlier than the start time so we need to remove them. Other columns day_of_week and month can also be helpful in the analysis of trips at different times in a year.
